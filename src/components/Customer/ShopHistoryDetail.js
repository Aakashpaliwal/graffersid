...
export class ShopHistoryDetail extends Component {
    constructor () {
        super() 
        this.state = {       
          userdata : [],
    ...
    handleClick () {
      this.state.shop_employe_name = localStorage.getItem('shop_employe_name')
      this.state.shop_total_bill = localStorage.getItem('shop_total_bill')
      this.state.shop_date_of_delivery = localStorage.getItem('shop_date_of_delivery')
        let productid = this.props.match.params.id
      axios.get(`${API_URL}user/more_detail/${productid}`,axios.defaults.headers.common['authorization'] = 'Bearer ' +	authService.getToken())
      .then(response => {
                console.log('More Detial Here')
                console.log(response)
                console.log(response.data.user_product_data);
                
                const data = response.data.user_product_data;
   ...
   <thead className="custom-table-head-color">
     ...
     <th scope="col">Total Bill Payment</th>
     <th scope="col">Date Of Delivery</th>
     </tr>
   </thead>
   <tbody>
         <tr>
            <th scope="row">{this.state.shop_employe_name}</th>
            <th scope="row">{this.state.shop_total_bill}</th>
            <th scope="row">{this.state.shop_date_of_delivery}</th>
         </tr>
              </tbody>
              </table>
                    </div>
