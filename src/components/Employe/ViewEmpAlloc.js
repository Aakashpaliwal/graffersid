...
export class ViewEmpAlloc extends Component {
  constructor() {
    super();
    this.state = {
      userdata: [],
  ...
  prevpagehandler() {
    ...
    this.props.history.push(`/ViewEmpAlloc/${this.props.match.params.id}`);
    this.forceUpdate();
    this.handleClick();
    // axios
    //   .get(
    //     `${API_URL}product_employe/view/${dataskip}/${limit}`,
    //     (axios.defaults.headers.common["w_auth"] = authService.getToken())
    //   )
    //   .then(response => {
    //     console.log(response.data.product_employe_data);
    //     const data = response.data.product_employe_data;
  ...
  handleClick() {
    ...
    axios
      .get(
        `${API_URL}product_employe/view/${defaultskip}/${defaultlimit}`,
        (axios.defaults.headers.common["authorization"] =
          "Bearer " + authService.getToken())
      )
      .then(response => {
              ...
              {this.state.userdata ? (
                this.state.userdata.map(function(item, id) {
                  return (
                    <tr key={id}>
                      <td>{++sr}</td>
                      <th scope="row" className="emp-alloc-name">
                        {item.employe_name}
                      </th>
                      ...
                      <td>{item.date_of_given_time}</td>
                      <td>{item.quantity_given} Pieces</td>
                      {/* <td>{todo.rate_given}</td>
       <td>{todo.total_rate_given}</td> */}
                      {/* <td>{todo.payment_received}</td>
       <td>{todo.total_payment}</td> */}
                      {/* <td>
